...
namespace io.github.toyota32k.dotnet6.toolkit.view {
    public class CircleProgressView : UserControl {
        ...
        private Size ArcSize => new(Radius, Radius);
        private string ProgressText => $"{Progress}%";

        /**
         * 円弧の始点
         */
        private Point StartPoint => new(50, RingThickness / 2);
        ...
        public CircleProgressView() {
            this.Content = Create(new Grid(), (rootGrid) => {
                rootGrid.Children.Add(Create(new Viewbox(), (viewBox) => {
                    viewBox.Child = Create(new Grid() { Width = 100, Height = 100, Background = new SolidColorBrush(Colors.Transparent) }, (grid) => {
                        grid.Children.Add(Create(new Ellipse() { Width = 100, Height = 100 }, (ellipse_) => {
                            this.ellipse = ellipse_;
                            this.ellipse.Stroke = RingStrokeColor;
                            this.ellipse.StrokeThickness = RingThickness;
                            this.ellipse.Fill = RingFillColor;
                        }));
                        grid.Children.Add(Create(new Path(), (path_) => {
                            this.path = path_;
                        ...
                        grid.Children.Add(Create(new TextBlock() { HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center, FontSize = 30, FontWeight = FontWeight.FromOpenTypeWeight(500) }, (label_) => {
                            this.label = label_;
                            this.label.Text = ProgressText;
            ...
            CreateBinding(label!, VisibilityProperty, this, "ShowLabel", new BoolVisibilityConverter());
            CreateBinding(label!, TextBlock.ForegroundProperty, this, "LabelColor");
            CreateBinding(ellipse!, Shape.StrokeProperty, this, "RingStrokeColor");
            CreateBinding(ellipse!, Shape.FillProperty, this, "RingFillColor");
            CreateBinding(ellipse!, Shape.StrokeThicknessProperty, this, "RingThicknessRatio", new RingThicknessConverter());
            CreateBinding(path!, Shape.StrokeProperty, this, "ProgressStrokeColor");
            CreateBinding(path!, Shape.StrokeThicknessProperty, this, "RingThicknessRatio", new RingThicknessConverter());
        }
